    ...
    for record in chunkify(handle):
        ...
        try:
            ...
            if fields["functionality"] == "F" and not fields["partial"].strip() and not fields["reverse"].strip():
                ...
                if fields["region"].upper() == region_name.upper():
...
def format_j_genes(jalignments, file_path):

    reference = ("WFAYWGQGTLVTVSA", 4, 19)
    #                 seq           start  end

    ffile = write_fasta(jalignments, file_path)
...
def combine_sequences(vsequences, jsequences):
    ...
    for v in vsequences:
        ...
        for j in jsequences:
            _, jallele = j
            combined_sequences[("%s_%s_%s" % (vspecies, vallele, jallele)).replace(" ", "_")] = (
...
def make_putative_alignments(vsequences, jsequences, calignments=None):
    ...
    for species, chain_type in vsequences:
        if (species, chain_type) not in jsequences or (species, chain_type) not in vsequences:
            print("fuck")
            continue
                ...
                try:
                    ...
                    try:
                        all_gene_alignments["V"][chain_type] = {
                            translations[species]: {gene: seq.replace(".", "-") + "-" * 20}
                        }
                    except KeyError:
                        all_gene_alignments["V"] = {
                            chain_type: {translations[species]: {gene: seq.replace(".", "-") + "-" * 20}}
                        }
                ...
                try:
                    ...
                    try:
                        all_gene_alignments["J"][chain_type] = {
                            translations[species]: {gene: "-" * 108 + seq.replace(".", "-")}
                        }
                    except KeyError:
                        all_gene_alignments["J"] = {
                            chain_type: {translations[species]: {gene: "-" * 108 + seq.replace(".", "-")}}
                        }
...
def main():
    ...
    jalignments = format_j_genes(jalignments, file_path)
    ...
    all_jalignments = format_j_genes(all_jalignments, file_path)
